...
for line, station_str in lines.items():
    for station_str in [s.strip() for s in station_str.split('-')]:
        station_list.append((line, station_str))

valid_pairs = set()
while len(valid_pairs) < 100:
    s1, s2 = random.sample(station_list, 2)
    if s1[1] != s2[1]:
        valid_pairs.add((s1, s2))

df = pd.DataFrame(valid_pairs, columns=['start_station', 'end_station'])
